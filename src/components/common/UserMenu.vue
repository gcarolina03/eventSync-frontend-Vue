<template>
  <div class="flex items-center md:order-2 relative">
    ...
    <div class="z-50 absolute top-5 right-2 my-4 text-base list-none bg-white rounded-lg shadow" :class="{ hidden: !menu }">
      ...
      <ul class="py-2">
        <li>
          <router-link to="/profile" class="block px-4 py-2 font-bold hover:text-primary">
            {{ $t('userMenu.profile') }}
          </router-link>
        </li>
        <li>
        ...
        <li>
          <router-link to="/profile/services" class="block px-4 py-2 font-bold hover:text-primary">
